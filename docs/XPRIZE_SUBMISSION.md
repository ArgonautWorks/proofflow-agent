...
ProofFlow is deployed at <https://proofflow-agent.vercel.app>. A public health request performs a server-authenticated Firestore write before reporting that Firestore is connected. Production audit `IxlFsvwqGZJ8ZHhQJ9dy` is available as [JSON](https://proofflow-agent.vercel.app/api/runs/IxlFsvwqGZJ8ZHhQJ9dy) and a generated [Markdown evidence pack](https://proofflow-agent.vercel.app/api/runs/IxlFsvwqGZJ8ZHhQJ9dy/report); it scored 96/100 and records a successful `gemini-3.6-flash` audit plus `gemma-4-26b-a4b-it` function-bound priority selection. The newer audit `C79YwCQLkC8VkqOU0Ezf` is also available as [JSON](https://proofflow-agent.vercel.app/api/runs/C79YwCQLkC8VkqOU0Ezf) and a [Markdown evidence pack](https://proofflow-agent.vercel.app/api/runs/C79YwCQLkC8VkqOU0Ezf/report); it scored 95/100 and records the live `gemini-3.5-flash-lite` availability fallback, successful `gemma-4-26b-a4b-it` priority selection, and `gemini-embedding-2` grounding at 0.7463 cosine similarity to an exact validated risk. A [passing external production trace](https://github.com/ArgonautWorks/proofflow-agent/actions/runs/31054527193) generated and verified that record. Together the two immutable records demonstrate all four deployed Google model paths without claiming they succeeded simultaneously. The current public repository contains 28 passing tests plus lint and production-build checks.
